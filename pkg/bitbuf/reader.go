...
func (buf *Reader) ReadBool() (bool, error) {
	v, err := buf.readInternal(1)
	return v == 1,err
}
...
func (buf *Reader) ReadFloat(min, max float32, numBits uint) (float32, error) {
	i := 1 << numBits - 1
	bits, err := buf.ReadInt32Bits(numBits)
	if err != nil {
		return 0, err
	}
	return min + float32(bits) / float32(i) * (max - min), nil
}
